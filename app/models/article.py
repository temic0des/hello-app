...
class Tag(db.Model):
    ...
    articles: so.WriteOnlyMapped["Article"] = so.relationship(secondary='article_tag',
        back_populates='tags')
...
class Article(db.Model):
    ...
    source_id: so.Mapped[str] = so.mapped_column(ForeignKey("article_sources.id"), index=True, nullable=True)
    ...
    tags: so.WriteOnlyMapped[Tag] = so.relationship(secondary=article_tag, 
...
class ArticleSource(db.Model):
    ...
    sources: so.WriteOnlyMapped[Article] = so.relationship(
